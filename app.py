# app.py
# Aplikasi Rekap Gaji Karyawan
# - Freeze kolom NIP & Nama di semua tabel
...
def with_pinned(base_config, df_like) -> dict:
    ...
    if df_like is not None and hasattr(df_like, "columns"):
        ...
        if 'nip' in cols:
            try:
                if pd.api.types.is_numeric_dtype(df_like['nip']):
                    cfg['nip'] = st.column_config.NumberColumn("NIP", pinned="left")
                else:
                    cfg['nip'] = st.column_config.TextColumn("NIP", pinned="left")
            except Exception:
                cfg['nip'] = st.column_config.TextColumn("NIP", pinned="left")
        if 'nama' in cols:
            cfg['nama'] = st.column_config.TextColumn("Nama", pinned="left")
    if base_config:
        for k, v in base_config.items():
            cfg[k] = v
    return cfg

def ensure_cols_str(df, cols=('nip',)):
...
def _parse_uploaded_bytes(file_bytes: bytes, filename: str) -> pd.DataFrame:
    ...
    # Jangan baca scan pada hari sakit
    ...
    for _, group in scanlog_df.sort_values('tanggal').groupby(group_keys, dropna=False):
        ...
        if has_ist and jumlah_hari_hadir > 0:
            for idx, row in group.loc[hadir_mask].iterrows():
                ...
                potongan_istirahat += pot
                istirahat_kejadian += 1
                istirahat_total_menit_excess += int(round(menit_excess, 0))

        potongan_istirahat = int(round(potongan_istirahat, 0))
        ...
        total_potongan = (
            int(potongan_terlambat) +
            int(potongan_absen) +
            int(potongan_izin_parsial) +
            int(potongan_istirahat)
        )
        ...
        if istirahat_kejadian > 0:
            alasan_parts.append(f"Istirahat lebih: {istirahat_kejadian} kejadian / {istirahat_total_menit_excess} menit excess (Rp {potongan_istirahat:,})")
        if sakit_skd > 0:
        ...
        results.append({
            ...
            "istirahat_lebih_kejadian": int(istirahat_kejadian),
            "istirahat_lebih_menit": int(istirahat_total_menit_excess),
            "potongan_istirahat_lebih": int(potongan_istirahat),
...
def build_rajin_recap(
    ...
    rekap_rajin['status'] = rekap_rajin.apply(lambda r: 'Rajin' if r['jumlah_hari_kerja'] == r['jumlah_bersih'] else 'Tidak', axis=1)
...
def main():
    ...
    with st.expander("🎛️ Filter Data", expanded=True):
        ...
        f_jabatan_default = _keep_valid(prev_jab, jab_opsi_all)
    ...
    _df = _df_date.copy()
    if f_dept: _df = _df[_df['departemen'].isin(f_dept)]
    if f_jabatan: _df = _df[_df['jabatan'].isin(f_jabatan)]
    if f_nama: _df = _df[_df['nama'].isin(f_nama)]
    if f_kategori: _df = _df[_df['jadwal_kategori'].isin(f_kategori)]
    ...
    with st.expander("🔍 Preview Data Scanlog (setelah filter)"):
        ...
        with colC:
            ...
            if 'tanggal' in _df.columns and _df['tanggal'].notna().any():
                _tmin = pd.to_datetime(_df['tanggal'].min()); _tmax = pd.to_datetime(_df['tanggal'].max())
        ...
        st.dataframe(df_prev, use_container_width=True, column_config=with_pinned({}, df_prev))

    # ========== Gaji Per Hari editor ==========
    karyawan_master = _df[['nip','nama','jabatan','departemen']].drop_duplicates().reset_index(drop=True)
    karyawan_master['nip'] = karyawan_master['nip'].astype(str)
    ...
    with tab_status:
        ...
        if status_view_df.empty:
            ...
            status_view_df_ed = ensure_cols_str(status_view_df, ('nip',))
            edited_status = st.data_editor(
                status_view_df_ed[cols_show],
                use_container_width=True,
                column_config=with_pinned({
    ...
    with tab_izin:
        ...
        if izin_editor_df.empty:
            ...
            izin_editor_df_ed = ensure_cols_str(izin_editor_df, ('nip',))
            edited_parsial = st.data_editor(
                izin_editor_df_ed[shown_cols],
                use_container_width=True,
                column_config=with_pinned({
                    "tanggal_fmt": st.column_config.TextColumn("Tanggal (dd-mm-yy)"),
    ...
    with tab_gph:
        ...
        if 'gph_df' not in st.session_state or st.session_state.gph_df is None or st.session_state.gph_df.empty:
            base = master_view.copy(); base['gaji_per_hari'] = int(default_gph)
            st.session_state.gph_df = base
        else:
        ...
        view_ed = ensure_cols_str(view, ('nip',))
        st.write(f"Baris tampil: **{len(view_ed):,}** (mengikuti filter utama)")
    ...
    # Map gaji per hari
    gph_map = {}
    if 'gph_df' in st.session_state and not st.session_state.gph_df.empty:
    ...
    # Trigger compute bila filter berubah
    filter_key = "|".join([
    ...
    with tab_rekap:
        ...
        if rekap_df is None or rekap_df.empty:
            ...
            display_columns = [
                ...
                'istirahat_lebih_kejadian','istirahat_lebih_menit','potongan_istirahat_lebih',
                'total_potongan','gaji_akhir','alasan_potongan'
            ]
            display_columns = [c for c in display_columns if c in _rd.columns]
            rd_view = _rd[display_columns]

            # Tabel tanpa filter lokal
            st.dataframe(rd_view, use_container_width=True, column_config=with_pinned({}, rd_view))

            # Ringkasan (mengikuti Filter Data utama saja)
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                total_karyawan = int(_rd['nip'].nunique()) if 'nip' in _rd.columns else 0
            ...
            # Download (menghormati Filter Data utama)
            st.divider()
            st.subheader("⬇️ Download")
            rekap_buf = io.BytesIO()
            with pd.ExcelWriter(rekap_buf, engine='openpyxl') as writer:
                rd_view.to_excel(writer, sheet_name='Rekap_Per_Karyawan', index=False)
            rekap_buf.seek(0)
            st.download_button(
            ...
            # Statistik ringkas (juga tanpa filter lokal)
            stat_buf = io.BytesIO()
            stat_global_rows = [
                ["Total Karyawan", total_karyawan],
                ["Total Potongan", int(_rd['total_potongan'].sum()) if 'total_potongan' in _rd.columns else 0],
                ["Total Gaji Bruto (hadir)", int(_rd['gaji_bruto'].sum()) if 'gaji_bruto' in _rd.columns else 0],
                ["Total Gaji Akhir", int(_rd['gaji_akhir'].sum()) if 'gaji_akhir' in _rd.columns else 0],
            ]
            ...
            with pd.ExcelWriter(stat_buf, engine='openpyxl') as writer:
                ...
                if not df_per_dept.empty: df_per_dept.to_excel(writer, sheet_name='Per_Departemen', index=False)
                if not df_per_jabatan.empty: df_per_jabatan.to_excel(writer, sheet_name='Per_Jabatan', index=False)
                if not df_telat.empty: df_telat.to_excel(writer, sheet_name='Telat_Per_Karyawan', index=False)
    ...
    with tab_output:
        ...
        if rekap_df is None or rekap_df.empty:
            ...
            kolom_output = [
                'nip','nama','jabatan','departemen',
                'gaji_per_hari','hari_hadir','gaji_bruto',
                'potongan_terlambat','potongan_absen','potongan_izin_pribadi_parsial','potongan_istirahat_lebih',
                'total_potongan','gaji_akhir'
            ]
            kolom_output = [c for c in kolom_output if c in ak.columns]
            out_view = ak[kolom_output]
            st.dataframe(out_view, use_container_width=True, column_config=with_pinned({}, out_view))

            c1, c2 = st.columns(2)
            with c1:
                st.metric("Total Karyawan", ak['nip'].nunique() if 'nip' in ak.columns else 0)
            with c2:
                st.metric("Total Jumlah Pendapatan (Gaji Akhir)", f"Rp {int(ak['gaji_akhir'].sum()) if 'gaji_akhir' in ak.columns else 0:,}")

            out_buf = io.BytesIO()
            with pd.ExcelWriter(out_buf, engine='openpyxl') as writer:
                out_view.to_excel(writer, sheet_name='Output_Akhir', index=False)
            out_buf.seek(0)
    ...
    with tab_rajin:
        ...
        rekap_rajin, detail_tidak_rajin = rekap_rajin_cache, detail_tidak_rajin_cache
        ...
        if rekap_rajin is None or rekap_rajin.empty:
            ...
            if 'departemen' in _r.columns: cols_rekap.append('departemen')
            cols_rekap += ['jumlah_hari_kerja','jumlah_bersih','status']
            r_view = _r[cols_rekap]
            st.markdown("### 📋 Rekap Rajin")
            st.dataframe(r_view, use_container_width=True, column_config=with_pinned({}, r_view))

            buf_r = io.BytesIO()
            with pd.ExcelWriter(buf_r, engine='openpyxl') as writer:
                r_view.to_excel(writer, sheet_name='Rekap_Rajin', index=False)
            buf_r.seek(0)
            ...
            if detail_tidak_rajin is None or detail_tidak_rajin.empty:
                ...
                if 'departemen' in detail_tidak_rajin.columns: cols_det.append('departemen')
                ...
                dtr_view = detail_tidak_rajin[cols_det]
                st.dataframe(dtr_view, use_container_width=True, column_config=with_pinned({}, dtr_view))

                buf_d = io.BytesIO()
                with pd.ExcelWriter(buf_d, engine='openpyxl') as writer:
                    dtr_view.to_excel(writer, sheet_name='Detail_Tidak_Rajin', index=False)
                buf_d.seek(0)
